	...
	(function () {
		...
		test.describe('Browser Test: ' + browser.capabilities[i].name, function () {
			...
			for (let d = 0; d < url.path.length; d++) {
				test.it('visit page', function (done) {
					driver.get('http://www.parliament.uk');

					driver.wait(webdriver.until.elementLocated(webdriver.By.xpath('//div[1]/div[1]/img'))).getCssValue('width').then(function (res) {
						console.log(res);
						chai.assert.isAtMost(res, '500', res + ' is less than or equal to 100')
					});
